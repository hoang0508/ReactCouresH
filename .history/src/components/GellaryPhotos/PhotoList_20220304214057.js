...
const PhotoList = () => {
  const { photos, cartItems } = useGellary();
  console.log(photos);
  console.log(
    "🚀 ~ file: PhotoList.js ~ line 6 ~ PhotoList ~ cartItems",
    cartItems
  );
  return (
...
const PhotoItem = ({ info: { url, id, isFavorite } }) => {
  ...
  const { toggleFavorite, addToCart } = useGellary();
  ...
  return (
    <>
      <div className="relative h-[300px] cursor-pointer group">
        ...
        <button
          className="absolute invisible px-6 py-2 text-black transition-all bg-white rounded-lg opacity-0 bottom-5 left-2/4 -translate-x-2/4 group-hover:opacity-100 group-hover:visible"
